...
export function CalendarConnectionPanel({ timeMin, timeMax, onEvents }: Props) {
  ...
  const disconnect = async () => { if (!window.confirm('Ginji OS内のCalendar接続情報を削除します。Googleログイン状態は維持されます。Google側のアクセス権解除に失敗した場合は、Googleアカウント設定から取り消せます。続けますか？')) return; setSaving(true); setError(null); try { const result = await deleteCalendarConnection(); setConnection({ connected: false, connectedAt: null, selectedCalendarIds: [], needsReconnect: false, canWriteEvents: false }); setCalendars([]); setSelectedIds([]); onEvents([]); if (!result.googleRevoked) setError('Ginji OS内の接続は解除しました。Google側の権限はGoogleアカウント設定から取り消してください。'); } catch (deleteError) { setError(deleteError instanceof Error ? deleteError.message : '接続を解除できませんでした。'); } finally { setSaving(false); } };

  return <section className="rounded-3xl border border-blue-200 bg-white p-4 shadow-sm" aria-label="Google Calendar接続">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-lg font-semibold">Google Calendar</h3><p className="mt-1 text-sm text-slate-600">予定を読み取り、明示確認後だけ作業予定を作成します。既存予定の変更・削除は行いません。</p></div>{connection?.connected && !connection.needsReconnect ? <span className={`rounded-full px-3 py-1 text-xs font-semibold ${connection.canWriteEvents ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-900'}`}>{connection.canWriteEvents ? '追加権限あり' : '読み取りのみ'}</span> : null}</div>
    ...
    {!loading && (!connection?.connected || connection.needsReconnect) ? <div className="mt-4"><p className="text-sm text-slate-600">{connection?.needsReconnect ? '認証が期限切れです。再接続してください。' : '接続操作時だけCalendarの読み取り・予定追加権限を確認します。'}</p><button type="button" disabled={saving} onClick={() => void connect()} className="mt-3 min-h-11 rounded-full bg-blue-600 px-4 font-semibold text-white disabled:opacity-50">{connection?.needsReconnect ? 'Google Calendarを再接続' : 'Google Calendarを接続'}</button></div> : null}
    {!loading && connection?.connected && !connection.needsReconnect ? <div className="mt-4 space-y-3">
      {!connection.canWriteEvents ? <div className="rounded-xl bg-amber-50 p-3 text-sm text-amber-900"><p>予定を追加するには、Googleの追加同意が必要です。</p><button type="button" disabled={saving} onClick={() => void connect()} className="mt-2 min-h-10 font-semibold underline disabled:opacity-50">予定追加権限を許可する</button></div> : null}
