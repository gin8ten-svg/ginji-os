...
export function TaskDataProvider({ children }: { children: React.ReactNode }) {
  ...
  const updateLocal = useCallback((updater: (current: TaskStore) => TaskStore) => {
    ...
    setStore((current) => {
      ...
      try { localRepository.save(next); } catch { setError('変更を端末に保存できませんでした。'); }
      return next;
    });
  }, [localRepository]);

  const value = useMemo<TaskDataContextValue>(() => ({
    store, isLoading, isHydrated, isAuthenticated, error, recoveryNotice,
    saveTask: (task) => {
      if (!remoteRepository) {
    ...
    deleteTask: (id) => {
      ...
      void runRemote(async () => { await remoteRepository.deleteTask(id); setStore((current) => current ? ({ ...current, tasks: current.tasks.filter((task) => task.id !== id) }) : current); });
    },
    saveRoutine: (routine) => {
    ...
    deleteRoutine: (id) => {
      ...
      void runRemote(async () => { await remoteRepository.deleteRoutine(id); setStore((current) => current ? remove(current) : current); });
    },
    toggleRoutineCompletion: (routineId, date = tokyoDateKey()) => {
      ...
      void runRemote(async () => { await remoteRepository.setRoutineCompletion(routineId, date, completed); setStore((current) => current ? toggle(current) : current); });
    },
  }), [error, isAuthenticated, isHydrated, isLoading, recoveryNotice, remoteRepository, runRemote, store, updateLocal]);
