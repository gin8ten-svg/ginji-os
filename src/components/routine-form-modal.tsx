...
export function RoutineFormModal({ routine, isSubmitting = false, onSave, onClose }: RoutineFormModalProps) {
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setSubmitting(false);
    if (result.error) setValidationError(result.error); else onClose();
  };

  return (
    <ModalShell labelledBy="routine-form-title" onClose={onClose} closeDisabled={busy}>
        <div className="flex items-center justify-between"><h2 id="routine-form-title" className="text-xl font-semibold">{routine ? 'ルーティンを編集' : '新しいルーティン'}</h2><button type="button" disabled={busy} onClick={onClose} className="min-h-11 rounded-full px-4 text-sm font-medium text-slate-600 hover:bg-slate-100 disabled:opacity-50">閉じる</button></div>
        <form onSubmit={submit} className="mt-4 space-y-4">
          ...
          <button type="submit" disabled={busy} className="min-h-12 w-full rounded-xl bg-violet-600 px-4 font-semibold text-white hover:bg-violet-500 disabled:cursor-not-allowed disabled:opacity-50">{busy ? '保存中…' : routine ? '変更を保存' : 'ルーティンを作成'}</button>
        </form>
    </ModalShell>
  );
}
