...
export function PlannerPanel({ store, isAuthenticated }: { store: TaskStore; isAuthenticated: boolean }) {
  ...
  useEffect(() => {
    ...
    return () => { mounted.current = false; activeCoordinator.abort(); };
  }, []);
  ...
  async function calculate() {
    ...
    try {
      ...
      const calendar = await resolvePlanningCalendarInput(isAuthenticated, window, request.signal, { getConnection: getCalendarConnection, getEvents: getCalendarEvents });
      const next = buildPlanningResult({ now, events: calendar.events, tasks: store.tasks, routines: store.routines, completions: store.routineCompletions });
      if (mounted.current && coordinator.current.isCurrent(request.generation)) setResult({ ...next, warnings: calendar.warnings });
    } catch (calculationError) {
      if (mounted.current && coordinator.current.isCurrent(request.generation) && !request.signal.aborted) setError(calculationError instanceof Error ? calculationError.message : '計画案を作成できませんでした。');
    } finally {
      if (mounted.current && coordinator.current.isCurrent(request.generation)) setLoading(false);
      coordinator.current.finish(request.generation);
    }
  }
  ...
  return <section className="rounded-3xl border border-cyan-200 bg-white p-4 shadow-sm" aria-label="7日間の計画案">
    ...
    {result ? <div className="mt-4 space-y-4">
      {loading ? <p role="status" className="rounded-2xl bg-cyan-50 p-3 text-sm font-medium text-cyan-800">再計算中です…</p> : null}
      {result.warnings.map((warning) => <p key={warning} role="status" className="rounded-2xl bg-amber-50 p-3 text-sm font-medium text-amber-900">{warning} <a href="/calendar" className="underline">Calendar接続を確認</a></p>)}
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4"><Metric label="配置ブロック" value={`${result.proposedBlocks.length}件`} /><Metric label="Google予定" value={`${result.busyIntervals.filter((item) => item.source === 'google').length}区間`} /><Metric label="残り空き枠" value={`${result.freeSlots.length}枠`} /><Metric label="未配置" value={`${result.unscheduledTasks.length + result.unscheduledRoutines.length}件`} /></div>
      ...
      {result.unscheduledRoutines.length ? <section className="rounded-2xl bg-violet-50 p-4"><h4 className="font-semibold text-violet-900">配置できなかったルーティン</h4><ul className="mt-2 space-y-2">{result.unscheduledRoutines.map((routine) => <li key={`${routine.routineId}:${routine.targetDate}`} className="text-sm text-violet-900"><span className="font-semibold">{routine.title}</span>（{routine.targetDate}）— {routine.reason}</li>)}</ul></section> : null}
    </div> : null}
  </section>;
}
