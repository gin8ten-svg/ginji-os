...
export async function authenticatedCalendarContext() {
  ...
  if (userError || !userData.user) return { ok: false, response: calendarJson({ error: '認証が必要です。', code: 'AUTH_REQUIRED' }, 401) } as const;
...
export async function calendarAccessContext() {
  ...
  try {
    ...
    if (error instanceof CalendarReconnectError) {
      ...
      return { ok: false, response: calendarJson({ error: 'Google Calendarへの再接続が必要です。', code: 'RECONNECT_REQUIRED', needsReconnect: true }, 409) } as const;
    }
...
export function calendarErrorResponse(error: unknown, code?: 'CALENDAR_FETCH_FAILED') {
  const message = error instanceof Error && error.message.startsWith('Google Calendar') ? error.message : 'Google Calendarの処理に失敗しました。';
  return calendarJson({ error: message, ...(code ? { code } : {}) }, 400);
}
