...
function providerError(error: unknown): PlanningApiError {
  ...
  if (candidate.name === 'APIConnectionTimeoutError' || candidate.name === 'AbortError') return new PlanningApiError('AI_TIMEOUT', 'AIから時間内に応答がありませんでした。', 504);
...
export class OpenAIPlanningAdvisor implements PlanningAdvisor {
  ...
  async advise(input: PlanningAdviceInput): Promise<PlanningAdvice> {
    try {
      ...
      if (refused || !response.output_text) throw new PlanningApiError('AI_INVALID_RESPONSE', 'AIから有効な改善案を取得できませんでした。', 502);
      try { return JSON.parse(response.output_text) as PlanningAdvice; }
      catch { throw new PlanningApiError('AI_INVALID_RESPONSE', 'AIから有効な改善案を取得できませんでした。', 502); }
    } catch (error) { if (error instanceof PlanningApiError) throw error; throw providerError(error); }
  }
}
