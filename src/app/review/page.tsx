...
import { AiAdviceUsagePanel } from '@/components/ai-advice-usage-panel';
import { EmptyState } from '@/components/empty-state';
import { EstimationAccuracy } from '@/components/estimation-accuracy';
import { ErrorState } from '@/components/error-state';
import { LoadingState } from '@/components/loading-state';
import { PlanningDailyReviewCard } from '@/components/planning-daily-review';
...
export default function ReviewPage() {
  ...
  return <div className="space-y-4">
    ...
    {!isLoading && summary ? <>
      {isAuthenticated ? <PlanningDailyReviewCard /> : null}
      {isAuthenticated ? <PlanningReview /> : null}
      {isAuthenticated ? <EstimationAccuracy /> : null}
      {isAuthenticated ? <AiAdviceUsagePanel /> : null}
