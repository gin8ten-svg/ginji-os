...
export default function CalendarPage() {
  ...
  const [yearMonth, setYearMonth] = useState(() => ({ year: Number(today.slice(0, 4)), month: Number(today.slice(5, 7)) }));
  const [selected, setSelected] = useState(today);
  const dates = useMemo(() => monthGrid(yearMonth.year, yearMonth.month), [yearMonth]);
  ...
  const selectedRoutines = useMemo(() => store?.routines.filter((routine) => isRoutineScheduled(routine, selected)) ?? [], [selected, store?.routines]);
  const completionSet = new Set(store?.routineCompletions.map((item) => `${item.date}:${item.routineId}`) ?? []);

  const moveMonth = (offset: number) => setYearMonth((current) => monthShift(current.year, current.month, offset));
  const backToToday = () => { setSelected(today); setYearMonth({ year: Number(today.slice(0, 4)), month: Number(today.slice(5, 7)) }); };
