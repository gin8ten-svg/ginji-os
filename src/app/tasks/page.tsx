...
export default function TasksPage() {
  const { store, isLoading, error, saveTask, deleteTask, saveRoutine, deleteRoutine } = useTaskData();
  ...
  return (
    <div className="space-y-4">
      <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="mt-4 flex gap-2 overflow-x-auto pb-1" role="tablist" aria-label="タスク分類">{tabs.map((tab) => <button type="button" role="tab" aria-selected={activeTab === tab} key={tab} onClick={() => setActiveTab(tab)} className={`min-h-10 shrink-0 rounded-full px-3 text-sm font-medium ${activeTab === tab ? 'bg-brand-600 text-white' : 'bg-slate-100 text-slate-700'}`}>{tab}</button>)}</div>
      </section>
