...
- **上記の修正後も、実時刻（秒付き）で計画作成すると依然として失敗することが判明**。`create_planning_session_v2`修正の直後にPostgresログで`new row for relation "planning_blocks" violates check constraint "planning_blocks_minute_aligned_check"`を確認。`src/lib/planner/engine.ts`の`createPlanningWindow`が`now`（秒・ミリ秒つき）をそのままwindow開始時刻として使っており、DBの分単位アライメント制約に違反していた。これは`3fd4244`以前から存在していた可能性のある、より基礎的な既存バグで、たまたま分の境界ちょうどで計画作成を行った場合以外は常に失敗していたと考えられる。`now`を次の分へ切り上げるよう修正（回帰テスト追加）。
