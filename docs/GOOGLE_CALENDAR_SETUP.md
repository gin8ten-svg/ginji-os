...
`CALENDAR_OAUTH_STATE_SECRET` は、追加同意を開始した認証ユーザーとcallback後のユーザーを照合する
10分間だけ有効なHttpOnly Cookieの署名専用秘密値です。32文字以上の独立したランダム値を設定し、
暗号鍵やOAuth Client Secretを流用しないでください。

Calendar APIレスポンスは `Cache-Control: private, no-store` です。接続解除時はGoogleのrevoke endpointを
best-effortで呼び出し、その成否にかかわらずGinji OS内の接続情報を削除します。
